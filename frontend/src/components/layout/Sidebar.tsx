...
function NavItem({
  ...
  return (
    <NavLink to={to} onClick={onClick} className="group relative block">
      {({ isActive }) => (
        <>
          <motion.div
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex h-screen shrink-0 select-none flex-col justify-between border-r border-white/[0.08] bg-[#06070C]/95 backdrop-blur-3xl transition-transform duration-300 md:relative md:translate-x-0",
          mobileOpen ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div>
          {/* Header */}
          <div className="relative flex h-20 items-center justify-between overflow-hidden border-b border-white/[0.08] px-3.5">
        ...
        {/* Collapse toggle positioned at top-20 for desktop only */}
        <motion.button
          whileHover={{
            scale: 1.15,
          ...
          className="absolute -right-3.5 top-20 z-50 hidden h-7 w-7 items-center justify-center rounded-full border border-cyan-500/30 bg-[#080910] text-cyan-400 shadow-[0_0_20px_rgba(34,211,238,.2)] backdrop-blur-xl transition-all md:flex"
        >
