...
import { UploadCloud, Loader2, Server, AlertTriangle } from 'lucide-react'
...
export default function UploadDropzone({ activeTab }: { activeTab: number }) {
  ...
  return (
    <div className="glass rounded-2xl overflow-hidden flex flex-col">
      <div className="bg-white/[0.02] border-b border-white/[0.05] p-4 flex justify-between items-center">
        <div className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-wider">
          <Server size={14} /> Storage Usage
        </div>
        <span className={cn(
          'text-[10px] font-bold px-2 py-0.5 rounded-md border font-mono',
          isLimitReached
